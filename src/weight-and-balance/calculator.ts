 ...
 */
export function generateWeightAndBalancePlacardData(datum: WeightAndBalanceDatum, 
    ...
    if(config.seatingType == SeatingConfiguration.TANDEM) {
        retval = calculateTwoSeater(datum, config, aircraftEmptyWeight, aircraftArm, xaft, nonLiftingPartsWeight, real_options);
    } else {
        retval = calculateSingleSeater(datum, config, aircraftEmptyWeight, aircraftArm, xaft, nonLiftingPartsWeight, real_options);
    }
 ...
 *    measurements permit a safe aircraft to fly. 
 */
export function updateWeightAndBalance(datum: WeightAndBalanceDatum, 
                                        config: AircraftConfiguration, 
 ...
 *    measurements permit a safe aircraft to fly. 
 */
export function calculateWeightAndBalance(datum: WeightAndBalanceDatum, 
                                          config: AircraftConfiguration, 
...
//console.log("measurements:\n" + JSON.stringify(measurements, null, 2) );
    ...
    // checksums:
    const checksum1 = g1 * datum.distanceFrontWheelToDatum + g2 * (datum.distanceFrontWheelToDatum + datum.distanceFrontWheelToRearWheel);
 ...
 */
function calculateWingWeights(config: AircraftConfiguration, measurements: WeightAndBalanceMeasurement, calculatePrimary: boolean): number {
    ...
    switch(config.wingPanelCount) {
        ...
        case 6:
            weight = measurements.wing1Weight + measurements.wing2Weight + 
                     (measurements.wing3Weight || 0) + (measurements.wing4Weight || 0) +
                     (measurements.wing5Weight || 0) + (measurements.wing6Weight || 0);
            break;
...
function calculateBaseCGLimits(
    ...
    xaft: number,
    nlpWeight: number,
    p1ArmRangePercentage?: number): SingleSeaterWeightAndBalanceResult {
...
function calculateSingleSeater(
    ...
    if((config.wingMaxBallastAmount || 0) > 0) {
        retval.allowedWingBallast = calculateWaterBallast(datum, retval.minPilotWeight, retval.maxPilotWeight, ge, config.wingMaxBallastAmount || 0, options);
    }

    if(config.cockpitBallastBlockCount > 0) {
        retval.cockpitBallast = calculateCockpitBallast(datum, 
                                                        ge,
                                                        retval.emptyCGArm, 
                                                        xaft, 
...
function calculateTwoSeater(
    ...
    xaft: number,
    nlpWeight: number,
    options: WeightAndBalanceOptions): TwoSeaterWeightAndBalanceResult {
    ...
    if((config.wingMaxBallastAmount || 0) > 0) {
        ...
        retval.allowedWingBallast = calculateWaterBallast(datum, retval.soloMinPilotWeight, total_max_pilot, ge, config.wingMaxBallastAmount || 0, options);
    }
...
function calculateTwoSeaterAdjustedWeights(
    ...
    nlpWeight: number, 
    tailWeights: number[],
    options: WeightAndBalanceOptions): Map<number, TwoSeaterPilotWeightTailBallastAdjustment> {
...
function calculateWaterBallast(
    ...
    options: WeightAndBalanceOptions): WeightAndBalanceBallastAmount[] {
    ...
    pilot_weight = minPilotWeight - (minPilotWeight % options.placardWeightIncremments) + options.placardWeightIncremments;
...
function calculateCockpitBallast(datum: WeightAndBalanceDatum, 
                                emptyWeight: number, 
                                emptyCGArm: number, 
                                xaft: number, 
                                p1ArmUsed: number,
                                blockCount: number, 
                                blockWeight: number): WeightAndBalanceCockpitBallast[] {
...
function calculateTwoSeaterP2(
    ...
    xaft: number, 
    nlpWeight: number,
    options: WeightAndBalanceOptions ): TwoSeatWeightRange[] {
    ...
    for(let i = 0; i < blocks.length; i++) {
        for(let j = 1; j <= blocks[i].maxBlockCount; j++) {
            ...
            if(!item) {
                ...
                // new combo.
                let total_blocks = 0;
                item.forEach(b => { total_blocks += b.blockCount});
